...
export function ProductsClient({ products, accessToken, canManage }: Props) {
  ...
  return (
    <div className="space-y-6">
            ...
            <button
              ...
              className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
                categoryFilter === cat
                  ...
                  : 'bg-white/[0.06] text-ash hover:bg-white/[0.10]'
              }`}
            >
              {cat === 'ALL' ? 'Todos' : cat}
            </button>
      ...
      {/* Products grid */}
      {filtered.length === 0 ? (
        <div className="rounded-xl border border-white/8 bg-white/[0.02] p-12 text-center text-sm text-ash">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((product) => (
            <div
              key={product.id}
              className={`rounded-xl border bg-white/[0.02] p-5 transition-all ${
                product.isActive
                  ? 'border-white/8'
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    {product.category && (
                      <span className="text-xs text-ash bg-white/[0.06] rounded px-2 py-0.5">
                        {product.category}
                      </span>
                    )}
                    {product.isLicensed && (
                      <span className="text-xs text-violet-400 bg-violet-500/15 rounded px-2 py-0.5">
                        Licenciado
                      </span>
                    )}
                  </div>
                ...
                {!product.isActive && (
                  <span className="text-xs text-ash/60 ml-2">Inativo</span>
                )}
              </div>
              ...
              <div className="flex items-end justify-between">
                <div>
                  ...
                  <p className="text-xs text-ash">por {product.unit}</p>
                </div>
                {canManage && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-md rounded-2xl border border-white/8 bg-background p-6 shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-ash mb-1">Nome *</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div>
                <label className="block text-sm font-medium text-ash mb-1">Descrição</label>
                <textarea
                  rows={2}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-ash mb-1">Preço (R$) *</label>
                  <input
                    type="number"
                    min={0}
                    step="0.01"
                ...
                <div>
                  <label className="block text-sm font-medium text-ash mb-1">Unidade</label>
                  <input
                    type="text"
                    value={form.unit}
              ...
              <div>
                <label className="block text-sm font-medium text-ash mb-1">Categoria</label>
                <input
                  type="text"
                  value={form.category ?? ''}
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                ...
                <span className="text-sm text-ash">
                  Produto licenciado{' '}
                  <span className="text-xs text-ash/60">(aparece em Developer → Licenças)</span>
                </span>
              </label>
              ...
              {editingId && (
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    className="h-4 w-4 rounded border-white/30 text-primary focus:ring-primary"
                  />
                  <span className="text-sm text-ash">Produto ativo</span>
                </label>
              )}
            </div>
