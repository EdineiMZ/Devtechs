...
/** Cache key — namespaced to projects-service so it doesn't clash
 *  with rh-service's cache on the same Redis instance. */
const permissionCacheKey = (userId: string): string =>
  `projects:user:permissions:${userId}`;

/** TTL — 5 minutes, matches the rest of the platform. */
const CACHE_TTL_SECONDS = 5 * 60;

/**
 * Shared permission resolver — used both by `PermissionGuard` (the
 * pre-handler authorization step) and by services that need a
 * runtime branching decision (e.g. "can this user mark a task as
 * DONE outside their assignment?"). Same shape as rh-service's
 * resolver — duplicated rather than packaged so each service stays
