...
| `REDIS_URL` | â€” | required for Socket.IO Redis adapter |
...
- GET POST /tickets â€” list/open (`support:tickets:view`)
- PATCH /tickets/:id â€” update (`support:tickets:manage`)
- POST /tickets/:id/assign â€” `support:tickets:assign`
- POST /tickets/:id/messages â€” REST send-message
- GET /tickets/:id/messages â€” message history
- GET POST PATCH /kb â€” knowledge base (`support:kb:edit`)
- Socket.IO `/support` â€” events `ticket:join`, `message:send`, `message:new`, `typing:*`
...
```bash
pnpm --filter @szdevs/support-service typecheck   # tsc --noEmit
pnpm --filter @szdevs/support-service lint        # eslint
pnpm --filter @szdevs/support-service test        # jest unit suite
```
