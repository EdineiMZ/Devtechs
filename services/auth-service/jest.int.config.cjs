/**
 * Integration test config — separate from any unit-jest config so it
 * can pick a different rootDir, longer timeouts, and a global setup
 * that lifts the Nest app once.
 *
 * Run: pnpm --filter @devtechs/auth-service test:int
 *
 * Pre-reqs:
...
module.exports = {
  ...
  testTimeout: 30_000,
  // Sequential — we share a single test database and rate-limit
